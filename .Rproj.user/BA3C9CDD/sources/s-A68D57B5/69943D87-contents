shinyServer(function(input, output, session) {
  ...
  # source("Functions/multiroot.R",local = TRUE)
   #source("Functions/findbetamupsi2.R",local = TRUE)
    ...
    priors$prior<<-list(a=fb$atotalbeta,b=fb$btotalbeta)
  ...
  output$PriorGenPlot2_true_zero <- plotly::renderPlotly({
    ...
    x<-seq(0,1,length.out = 2000)
    Prevalence<-dbeta(x = x,shape1 = fb$a,shape2 = fb$b)
    ProbZeroPrev<-dbeta(x = x,shape1 = fb_tau0$a,shape2 = fb_tau0$b)
    
    Sensitivity<-dbeta(x = x,shape1 = fb_SE$abeta,shape2 = fb_SE$bbeta)
    Specificity<-dbeta(x = x,shape1 = fb_SP$abeta,shape2 = fb_SP$bbeta)
    df1 <- data.frame(x, Prevalence,ProbZeroPrev)
    df2 <- data.frame(x, Sensitivity,Specificity)
    df1 <- gather(df1, func, val, -x)
    df2 <- gather(df2, func, val, -x)
    ...
    gg1 <- gg1 + scale_y_continuous(expand=c(0, 0))
    gg1 <- gg1 + scale_color_manual(name="Prior(s)", 
                                    values=c("#b2182b","#34182c"),
                                    labels=c("Prevalence","ProbZeroPrev"))
    gg1 <- gg1 + labs(x="Density", y="Probability distribution functions")
    gg1 <- gg1 + theme_bw()
    gg1 <- gg1 + theme(panel.border=element_blank())
    gg1 <- gg1 + theme(axis.line=element_line(size=0.15, color="#2b2b2b"))
  ...
  # output$APPreZero_Plot <- renderPlot({...})
  # output$MultAPpre_Plot <- renderPlot({...})
  # output$MultTRpre_Plot <- renderPlot({...})
  # output$MultAPpreZero_Plot <- renderPlot({...})
  ...
  #                                                    "Apparent prevalence"),
  #                  label = 'Do you want to model the true prevalence or the apparent prevalence?',
  #                  selected = "Apparent prevalence",inline = TRUE)
  #   }
  # })
...
output$downloadModel <- downloadHandler(
  filename <- function(){
    paste("Model1.mcmc_df.RData")
  },
  
  content = function(file) {
    save(Model1.mcmc_df, file = file)
  }
)
  ...
  output$Boxsetup3 <- shinydashboard::renderValueBox({
    shinydashboard::valueBox(
      paste0("Model setup:"),if(priors$PriorSelect==FALSE | priors$SetupPriors==FALSE | priors$mSetupmodel==FALSE){"Please set up the model, priors and input data in Tabs 'Set up', 'Priors' and 'Model' before you move forward"}else{"Model selected - go to report"},
